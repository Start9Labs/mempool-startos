...
const mempoolSection = z.object({
  ...
  // Upstream syslog-style priorities (backend/src/logger.ts). The wrapper
  // defaults to 'info' (upstream's in-source default is 'debug') and the
  // Indexing and Performance action exposes the debug/info/warn/err subset —
  // notably 'debug', which is the only level at which the block-summaries /
  // goggles / CPFP backfill logs per-block progress (issue #63).
