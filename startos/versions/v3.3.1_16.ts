...
- On every start with an indexing feature enabled, Mempool now logs that a backfill may be in progress, that intermittent 503 retry errors from Bitcoin are expected and non-fatal, and that restarting the service delays completion.

Also updates the bundled MariaDB database to 10.4.34, the final release of the 10.4 series — same series, so no database migration is needed. Mempool itself is unchanged at 3.3.1.`,
...
- En cada arranque con una función de indexación activada, Mempool ahora registra que un llenado puede estar en curso, que los errores 503 intermitentes de reintento de Bitcoin son esperados y no fatales, y que reiniciar el servicio retrasa su finalización.

Además, actualiza la base de datos MariaDB incluida a 10.4.34, la última versión de la serie 10.4 — la misma serie, por lo que no es necesaria ninguna migración de la base de datos. Mempool en sí se mantiene sin cambios en la versión 3.3.1.`,
...
- Bei jedem Start mit aktivierter Indexierungsfunktion protokolliert Mempool nun, dass ein Abgleich laufen kann, dass zeitweilige 503-Wiederholungsfehler von Bitcoin zu erwarten und nicht fatal sind, und dass ein Neustart des Dienstes den Abschluss verzögert.

Außerdem wird die mitgelieferte MariaDB-Datenbank auf 10.4.34 aktualisiert, die letzte Version der 10.4-Reihe — dieselbe Reihe, daher ist keine Datenbankmigration erforderlich. Mempool selbst bleibt unverändert bei 3.3.1.`,
...
- Przy każdym starcie z włączoną funkcją indeksowania Mempool loguje teraz, że uzupełnienie może być w toku, że sporadyczne błędy 503 z ponownymi próbami od Bitcoin są oczekiwane i niekrytyczne, oraz że ponowne uruchomienie usługi opóźnia jego zakończenie.

Ponadto aktualizuje dołączoną bazę danych MariaDB do wersji 10.4.34, ostatniego wydania serii 10.4 — ta sama seria, więc migracja bazy danych nie jest potrzebna. Sam Mempool pozostaje bez zmian w wersji 3.3.1.`,
...
- À chaque démarrage avec une fonction d'indexation activée, Mempool journalise désormais qu'un remplissage peut être en cours, que les erreurs 503 intermittentes de nouvelle tentative de Bitcoin sont attendues et non fatales, et que redémarrer le service retarde son achèvement.

De plus, met à jour la base de données MariaDB incluse vers la version 10.4.34, la dernière version de la série 10.4 — la même série, aucune migration de la base de données n'est donc nécessaire. Mempool lui-même reste inchangé en version 3.3.1.`,
  },
  migrations: {
    up: async ({ effects }) => {
