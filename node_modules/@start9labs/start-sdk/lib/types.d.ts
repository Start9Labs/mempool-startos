...
export declare namespace ExpectedExports {
    ...
    type properties = <WrapperData>(options: {
     ...
     */
    type autoConfig = Record<PackageId, AutoConfigure>;
}
...
export type ValidIfNoStupidEscape<A> = A extends `${string}'"'"'${string}` | `${string}\\"${string}` ? never : "" extends A & "" ? never : A;
...
export type Effects = {
    ...
    runCommand<A extends string>(command: ValidIfNoStupidEscape<A> | [string, ...string[]], options?: {
        timeoutMillis?: number;
    }): Promise<string>;
    runDaemon<A extends string>(command: ValidIfNoStupidEscape<A> | [string, ...string[]], options?: {
...
export type ExtractWrapperData<WrapperData, Path extends string> = Path extends `/${infer A}/${infer Rest}` ? (A extends keyof WrapperData ? ExtractWrapperData<WrapperData[A], `/${Rest}`> : never) : Path extends `/${infer A}` ? (A extends keyof WrapperData ? WrapperData[A] : never) : Path extends '' ? WrapperData : never;
type _EnsureWrapperDataPath<WrapperData, Path extends string, Origin extends string> = Path extends `/${infer A}/${infer Rest}` ? (WrapperData extends {
    [K in A & string]: infer NextWrapperData;
} ? _EnsureWrapperDataPath<NextWrapperData, `/${Rest}`, Origin> : never) : Path extends `/${infer A}` ? (WrapperData extends {
    [K in A]: infer B;
} ? Origin : never) : Path extends '' ? Origin : never;
