import { unionSelectKey, unionValueKey, } from "../config/configTypes";
import { matchConfigSpec } from "./output";
...
describe("Inputs", () => {
    ...
    test("test no longer care about the conversion of min/max and validating", () => {
        matchConfigSpec.unsafeCast(deepMerge({}, validInput, { rpc: { advanced: { threads: 0 } } }));
    });
    test("test errors should throw for number in string", () => {
        expect(() => matchConfigSpec.unsafeCast(deepMerge({}, validInput, { rpc: { enable: 2 } }))).toThrowError();
    });
    test("Test that we set serialversion to something not segwit or non-segwit", () => {
        expect(() => matchConfigSpec.unsafeCast(deepMerge({}, validInput, {
