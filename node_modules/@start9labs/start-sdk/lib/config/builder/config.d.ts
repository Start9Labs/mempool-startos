...
export type LazyBuild<WD, ExpectedOut> = (options: LazyBuildOptions<WD>) => Promise<ExpectedOut> | ExpectedOut;
export type ExtractConfigType<A extends Record<string, any> | Config<Record<string, any>, any> | Config<Record<string, any>, never>> = A extends Config<infer B, any> | Config<infer B, never> ? B : A;
...
export const addNodesSpec = Config.of({ hostname: hostname, port: port });
  ...
  ```
 */
export declare class Config<Type extends Record<string, any>, WD> {
    private readonly spec;
    validator: Parser<unknown, Type>;
    private constructor();
    build(options: LazyBuildOptions<WD>): Promise<{ [K in keyof Type]: ValueSpec; }>;
    static of<Spec extends Record<string, Value<any, any> | Value<any, never>>>(spec: Spec): Config<{ [K in keyof Spec]: Spec[K] extends Value<infer T, any> | Value<infer T, never> ? T : never; }, { [K_1 in keyof Spec]: Spec[K_1] extends Value<any, infer WD> ? WD : never; }[keyof Spec]>;
    /**
    ...
    ```
     */
    withWrapperData<NewWrapperData extends WD extends never ? any : WD>(): Config<Type, NewWrapperData>;
}
