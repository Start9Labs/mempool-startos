...
export async function oldSpecToBuilder(file, inputData, options) {
    await fs.writeFile(file, await makeFileContentFromOld(inputData, options), (err) => console.error(err));
}
function isString(x) {
    return typeof x === "string";
}
export default async function makeFileContentFromOld(inputData, { startSdk = "start-sdk", nested = true, wrapperData = "../../wrapperData", } = {}) {
    const outputLines = [];
    outputLines.push(`
...
`);
    ...
    const configMatcherName = newConst("matchConfigSpec", `${configName}.validator`);
    outputLines.push(`export type ConfigSpec = typeof ${configMatcherName}._TYPE;`);
    return outputLines.join("\n");
    function newConst(key, data, type) {
        const variableName = getNextConstName(camelCase(key));
        outputLines.push(`export const ${variableName}${!type ? "" : `: ${type}`} = ${data};`);
        return variableName;
    }
    function maybeNewConst(key, data) {
        if (nested)
            return data;
        return newConst(key, data);
    }
    function convertInputSpecInner(data) {
        let answer = "{";
    ...
    function convertInputSpec(data) {
        return `Config.of(${convertInputSpecInner(data)})`;
    }
    function convertValueSpec(value) {
        switch (value.type) {
            case "string": {
                if (value.textarea) {
                    return `${rangeToTodoComment(value?.range)}Value.textarea(${JSON.stringify({
                ...
                return `${rangeToTodoComment(value?.range)}Value.text(${JSON.stringify({
                    name: value.name || null,
                    // prettier-ignore
                    required: (value.default != null ? { default: value.default } :
            ...
            case "number": {
                return `${rangeToTodoComment(value?.range)}Value.number(${JSON.stringify({
                    name: value.name || null,
                    description: value.description || null,
                    warning: value.warning || null,
                    // prettier-ignore
            ...
            case "enum": {
                ...
                const values = Object.fromEntries(Array.from(allValueNames)
                    .filter(isString)
            ...
            case "object": {
                const specName = maybeNewConst(value.name + "_spec", convertInputSpec(value.spec));
                return `Value.object({
        ...
        warning: ${JSON.stringify(value.warning || null)},
            ...
            case "union": {
                const variants = maybeNewConst(value.name + "_variants", convertVariants(value.variants, value.tag["variant-names"] || {}));
                return `Value.union({
        name: ${JSON.stringify(value.name || null)},
    ...
    function convertList(value) {
        switch (value.subtype) {
            ...
            case "object": {
                const specName = maybeNewConst(value.name + "_spec", convertInputSpec(value.spec.spec));
          ...
          uniqueBy: ${JSON.stringify(value?.spec?.["unique-by"] || null)},
            ...
            case "union": {
                const variants = maybeNewConst(value.name + "_variants", convertVariants(value.spec.variants, value.spec["variant-names"] || {}));
                const unionValueName = maybeNewConst(value.name + "_union", `${rangeToTodoComment(value?.range)}
          Value.union({
        ...
        `);
                const listConfig = maybeNewConst(value.name + "_list_config", `
          Config.of({
    ...
    function convertVariants(variants, variantNames) {
    ...
    function getNextConstName(name, i = 0) {
...
function rangeToTodoComment(range) {
    if (!range)
        return "";
